...
public enum SortTeamStrategyEnum implements EnumInterface {
    SKILL_LEVEL(0, "Nível") {
        ...
        public List<TeamDto> sort(SortTeamDto dto) {
            ...
            this.addPlayersToTeams(dto.getPlayers().stream().filter(gkFilter).collect(toList()),
                dto.getAmount(),
                dto.getTeamSize(),
                teams,
            ...
            this.addPlayersToTeams(dto.getPlayers(),
                dto.getAmount(),
                dto.getTeamSize(),
                teams,
                teamsOrder);
    ...
    POSITION(1, "Posição") {
        ...
        public List<TeamDto> sort(SortTeamDto dto) {
            ...
            playersByPosition.forEach((pos, players) -> {
                ...
                addPlayersToTeams(players, dto.getAmount(), dto.getTeamSize(), teams, new ArrayList<>(teamsOrder));
            });
            teams.forEach(team -> team.getPlayers().sort(Comparator.comparing(Player::getPosition)));
            return teams;
        }
    ...
    @JsonCreator
    public static SortTeamStrategyEnum forValues(@JsonProperty("id") Integer id, @JsonProperty("name") String name) {
    ...
    void addPlayersToTeams(List<Player> players, Integer amount, Integer teamSize, List<TeamDto> teams, List<Integer> teamsOrder) {
        for (int i = 0; i < players.size(); ) {
            ...
            for (int j = 0; j < amount; j++) {
                ...
                i++;
                if (i < players.size() && PlayerPositionEnum.GK.isEqual(players.get(i).getPosition()) && j + 1 == amount) {
                    List<Player> benchPlayers = players.subList(i, players.size());
                    if (teams.size() == amount && !benchPlayers.isEmpty()) {
                        teams.add(TeamDto.builder().name("Reserva").players(benchPlayers).build());
                    } else if (!benchPlayers.isEmpty()) {
